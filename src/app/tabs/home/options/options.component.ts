...
export class OptionsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.categorySelectSubscription =
      this.databaseService.categorySelectSubject.subscribe((index) => {
        this.selectedCategoryIndex = index;
        this.sharedService.vocableList =
          this.databaseService.getVocableList(index);
        ...
        if (localStorage.getItem('vocableList' + index) != null) {
          ...
          this.loadedVocableList = JSON.parse(
            localStorage.getItem('vocableList' + index)
          );
        }
  ...
  onSaveVocableList() {
    ...
    localStorage.setItem(
      'vocableList' + this.selectedCategoryIndex,
      JSON.stringify(this.vocableList)
    );
    this.loadedVocableList = JSON.parse(
      localStorage.getItem('vocableList' + this.selectedCategoryIndex)
    );
  }
